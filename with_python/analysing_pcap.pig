...
-- =========================================================
full_traffic = LOAD '$inputfile' using PigStorage(' ') AS (
    ts,
    ip_version:int,
...
-- STORE full_traffic_bps_pps INTO '$outputFolder/full_traffic_bps_pps' USING PigStorage(',', '-schema');
...
-- STORE (ORDER pkts_per_ip_dst BY packets DESC) INTO '$outputFolder/full_traffic_pkts_per_ip_dst' USING PigStorage(',', '-schema');
...
-- STORE (ORDER joined_full_traffic_ip_proto_desc BY occurrences DESC) INTO '$outputFolder/full_traffic_ip_proto' USING PigStorage(',', '-schema');
...
-- STORE (ORDER joined_sport_statistics BY packets DESC) INTO '$outputFolder/full_traffic_filter2_sport' USING PigStorage(',', '-schema');
...
-- STORE (ORDER joined_dport_statistics BY packets DESC) INTO '$outputFolder/full_traffic_filter2_dport' USING PigStorage(',', '-schema');
...
-- STORE full_traffic_filter2_bps_pps INTO '$outputFolder/full_traffic_filter2_bps_pps' USING PigStorage(',', '-schema');
...
-- STORE full_traffic_filter2_stats INTO '$outputFolder/full_traffic_filter2_stats' USING PigStorage(',', '-schema');
